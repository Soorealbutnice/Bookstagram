...
function getList() {
	 var msupport_no = $(".row").last().data("msupport_no");
	 if(msupport_no == null){
		 msupport_no = 0;
	 } 
	 var parameter = {"msupport_no" : msupport_no};
	 $.ajax({
		url : '${root}/mypage/msupport.dnb',
		type : 'GET',
...
function makeList(msupport) {
	var mslist = msupport.msupportList;
	var output = "";
	for(var i=0;i<mslist.length;i++){
		output +=  '<div class="row" data-msupport_no="'+mslist[i].support_no+'">';
		output +=  ' <div class="col-lg-8">';
		output +=   '<h2><a href="javascript:mvMyPage(\''+mslist[i].supporter+'\');" style="color:black">'+mslist[i].supporter+'</a>님에게</h2>';
		output +=    '<h4><label>'+mslist[i].pay+'원</label>후원받았습니다.</h4><br>';
		output +=    ' <label>'+mslist[i].support_date+'</label>';
		output +=  ' </div>';
		output +=   ' <div class="col-lg-4 ">';
		output +=     ' <img class="rounded-circle" src="${root}/upload/profile/'+mslist[i].saveFolder+'/'+mslist[i].save_proPicture+'" width="170" height="170">';
		output +=  '  </div>';
		output += ' </div>';
		output += '<br><hr><br>';
	}
