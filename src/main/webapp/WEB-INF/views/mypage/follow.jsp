...
function getList() {
	 var follow_no = $(".row").children().last().data("follow_no");
	 if(follow_no == null){
	 	 follow_no = 0;
	 } 
	 var follow_id = '${member.id}';
	 var parameter = {"follow_no" : follow_no, "follow_id" : follow_id};
	 $.ajax({
		url : '${root}/mypage/follow.dnb',
		type : 'GET',
...
function makeList(follow) {
	var flist = follow.followList;
	var output = "";
	for(var i=0;i<flist.length;i++){
		output +=	'<div class="col-lg-3 follow" data-follow_no="'+flist[i].follow_no+'">';
		output +=	 ' <img class="rounded-circle" src="${root}/upload/profile/'+flist[i].saveFolder+'/'+flist[i].save_proPicture+'" width="140" height="140">';
		output +=	  '<a href="javascript:mvMyPage(\''+flist[i].followed_id+'\');""><h2>'+flist[i].nickname+'</h2></a>';
		output +=	  '<p style="color:#a5a5a5">'+flist[i].introduce+'</p>';  
		output +=	'</div>';
	}
	$("#followList").append(output);
}
